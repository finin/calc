...
/************************************************************************
* Defines a yacc grammar for a simple calculator using infix notation.
* When executed, the calculator enters a loop in which it prints the
* prompt >>, reads a toplevel expression terminated by a newline, and
* prints its value.  Operators include +, -, *, and =  (assignment). 
* All expressions return values, even assignment.  Parentheses can be
* used to override operator precedence and associativity rules. Based
* on zcalc by ruiz@capsl.udel.edu
************************************************************************/
...
/*
  ...
  deals with: doubles and pointers to symbol table entries.
*/
...
/* 
  grammar rules.  a session is a sequence of lines.  A toplevel is
  just an expr (print its value followed by two newlines and the
  prompt >>) or a '? (print help) or a '.' (exit).  An expr can be a
  number, name, the sum of two exprs, ...
 */
